...
public static class WritableOptionsCoreExtensions
{
    ...
    public static IServiceCollection AddWritableOptionsCore<T>(this IServiceCollection services, string instanceName)
        where T : class, new()
    {
        // add IOptionsConfigRegistry<T>
        services.AddSingleton<IOptionsConfigRegistry<T>, OptionsConfigRegistryImpl<T>>();
        ...
        {
            services.AddKeyedSingleton<IWritableOptions<T>>(instanceName, (p, key) =>
                new WritableOptionsWithNameImpl<T>(
                    p.GetRequiredService<WritableOptionsImpl<T>>(),
                    instanceName
                )
            );
            services.AddKeyedSingleton<IReadOnlyOptions<T>>(instanceName, (p, key) =>
