        ...
        IOptionsSchemaMetadataProvider
{
    ...
    public OptionsSchemaMetadata? ReadSchemaMetadata(IWritableOptionsConfiguration options)
    {
        ...
        try
        {
            ...
            var modelId = ReadOptionalString(
                current,
                OptionsSchemaMetadata.ModelIdPropertyName,
                serializerOptions
            );
            var version = ReadOptionalVersion(current, serializerOptions) ?? 1;
    ...
    {
        if (!TryGetMetadataProperty(element, propertyName, options, out var value))
        {
            return null;
        }
    ...
    private static int? ReadOptionalVersion(JsonElement element, JsonSerializerOptions options)
    {
        if (
    ...
    )
    {
        ...
        var convertedName = options.PropertyNamingPolicy?.ConvertName(propertyName);
    ...
    /// <inheritdoc />
    internal override object CreateDefault(Type type)
    {
        var typeInfo = GetEffectiveOptions().GetTypeInfo(type);
        return typeInfo.CreateObject?.Invoke()
